...
#include <astral_gazebo/KinematicsForward.h>
...
bool mobileToWorld(astral_gazebo::FrameToFrame::Request &request, astral_gazebo::FrameToFrame::Response &response) {
...
bool worldToMobile(astral_gazebo::FrameToFrame::Request &request, astral_gazebo::FrameToFrame::Response &response) {
    worldToMobileCore(request.input.x, request.input.y, response.output.x, response.output.y);
}

bool forwardMobile(astral_gazebo::KinematicsForward::Request &request, astral_gazebo::KinematicsForward::Response &response) {
    response.output.x     = ((2.0L * request.input.v_back_right) - request.input.v_front_left - request.input.v_front_right) / 3.0L;
    response.output.y     = ((sqrt3 * request.input.v_front_right) - (sqrt3 * request.input.v_front_left)) / 3.0L;
    response.output.theta = (request.input.v_front_left + request.input.v_back_right + request.input.v_front_right) / L3;
    return true;
}

bool forwardWorld(astral_gazebo::KinematicsForward::Request &request, astral_gazebo::KinematicsForward::Response &response) {
    forwardMobile(request, response);
    mobileToWorldCore(response.output.x, response.output.y, response.output.x, response.output.y);
    return true;
}

bool inverseMobile(astral_gazebo::KinematicsInverse::Request &request, astral_gazebo::KinematicsInverse::Response &response) {
    ...
    response.output.v_front_left  = V__m_x2 - sqrt3V__m_y2 + Lomega_p;
    response.output.v_back_right  = request.input.x        + Lomega_p;
    response.output.v_front_right = V__m_x2 + sqrt3V__m_y2 + Lomega_p;
    return true;
}

bool inverseWorld(astral_gazebo::KinematicsInverse::Request &request, astral_gazebo::KinematicsInverse::Response &response) {
